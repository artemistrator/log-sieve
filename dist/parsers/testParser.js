...
const locationPattern = /^\s*(?:at|❯|›)\s+(?:.+?\()?(?<file>[^():]+(?:\.[^():]+)?):(?<line>\d+):(?<column>\d+)\)?/;
const assertionPattern = /^\s*(?<message>(?:AssertionError|TypeError|ReferenceError|RangeError|SyntaxError|Error):.+|expected .+ to .+)$/i;
...
function sanitizeTestFile(value) {
    return value
        .trim()
        .replace(/\s+\(\d+(?:\.\d+)?\s*(?:ms|s)\)\s*$/, "")
        .replace(/\s+\[\s*.+\s*\]\s*$/, "");
}
function isClearlyTestAssertion(message) {
