...
const issuePattern =
  /^\s*(?<line>\d+):(?<column>\d+)\s+(?<category>error|warning)\s+(?<message>.+?)(?:\s{2,}(?<rule>[^\s]+))?\s*$/;

export function parseEslint(input: string): Issue[] {
  const issues: Issue[] = [];
  ...
  for (const line of input.split("\n")) {
    ...
    const { category, line: lineText, column: columnText, message, rule } =
      issueMatch.groups;
