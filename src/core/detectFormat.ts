...
type ParserFn = (input: string) => Issue[];

const parsers: Record<ToolName, ParserFn> = {
  tsc: parseTsc,
  eslint: parseEslint,
  test: parseTest,
...
const parserWeight: Record<ToolName, number> = {
  tsc: 6,
  test: 5,
  eslint: 4,
  generic: 1
...
export function detectFormat(input: string): ToolName {
  return selectBestFormat(input).tool;
}

export function selectBestFormat(input: string): { tool: ToolName; issues: Issue[] } {
  const knownResults = (Object.entries(parsers) as Array<[ToolName, ParserFn]>)
  ...
  const results = (Object.entries(parsers) as Array<[ToolName, ParserFn]>).map(([tool, parse]) => ({
    tool,
    issues: parse(input)
  }));
...
function selectDominantResult(results: Array<{ tool: ToolName; issues: Issue[] }>): {
  tool: ToolName;
  issues: Issue[];
} {
  const firstResult = results[0];
...
function getParseScore(tool: ToolName, issues: Issue[]): number {
  if (issues.length === 0) {
    return 0;
  }
...
function breaksTie(
  candidate: { tool: ToolName; issues: Issue[] },
  best: { tool: ToolName; issues: Issue[] }
): boolean {
